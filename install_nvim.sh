...
# Always use the Neovim from PATH (MSYS2 nvim), which has a correct runtime.
NVIM_CMD="nvim"
...
###############################################################################
# Clean up old packer plugins (we now use lazy.nvim)
###############################################################################
...
###############################################################################
# Disable upstream personal.keymaps so only our remaps are active
###############################################################################
...
if [[ -f "$INIT_LUA" ]]; then
    ...
    awk '
      /require.*personal\.keymaps/ && !done {
        print "-- " $0  # comment out the repo keymaps module
        done=1
        next
      }
      { print }
...
if [[ -f "$LAZY_PLUGINS_FILE" ]] && ! grep -q 'shaunsingh/moonlight.nvim' "$LAZY_PLUGINS_FILE" 2>/dev/null; then
    ...
    awk '
      # After custom.plugins.debug, inject moonlight + extras
...
local function on_attach(client, bufnr)
  ...
  nmap('<leader>vd', vim.diagnostic.open_float, 'Diagnostics Float')
...
echo "Done. Config from $REPO_URL with your keymaps and LSP layered on top (auto-session disabled, packer cleaned)."
